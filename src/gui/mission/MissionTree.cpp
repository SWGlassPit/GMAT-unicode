//$Id: MissionTree.cpp 9829 2011-09-01 17:56:21Z lindajun $
//------------------------------------------------------------------------------
//                              MissionTree
//------------------------------------------------------------------------------
                           ...
                           bool insertBefore)
{
   ...
   if (cmdAdded)
   {
               ...
               prevTypeName == wxT("Optimize") || prevTypeName == wxT("Else"))
      {
         ...
         #if DEBUG_MISSION_TREE_INSERT
         MessageInterface::ShowMessage
            (wxT("   previous type is NoOp, BranchCommand or Else\n"));
         MessageInterface::ShowMessage
            (wxT("   ==> inserting %s after %s\n"), nodeName.c_str(),
...
{
   #if DEBUG_MISSION_TREE_SHOW_CMD
   ShowCommands(wxT("Before Append: ") + cmdName);
   #endif
   ...
   if (cmdName != wxT("Else") && itemText.Contains(wxT("If")))
   {
   ...
   #if DEBUG_MISSION_TREE_APPEND
   MessageInterface::ShowMessage
      (wxT("   after FindChild(Else) itemId=<%s>, lastId=<%s>\n"),
   ...
   if (lastId.IsOk() && GetItemText(lastId) != wxT(""))
   ...
   if (cmd != NULL)
   {
      ...
      #if DEBUG_MISSION_TREE_APPEND
      MessageInterface::ShowMessage(wxT("   ==> Calling InsertCommand(before)\n"));
      #endif
   ...
   #if DEBUG_MISSION_TREE_SHOW_CMD
   ShowCommands(wxT("After Append: ") + cmdName);
   #endif
