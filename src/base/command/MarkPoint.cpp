//$Id: MarkPoint.cpp 9826 2011-08-31 22:16:42Z lindajun $
//------------------------------------------------------------------------------
//                                MarkPoint
//------------------------------------------------------------------------------
...
bool MarkPoint::InterpretAction()
{
   ...
   {
      wxString msg = 
         wxT("The MarkPoint command is not allowed to contain brackets, braces, or ")
         wxT("parentheses");
   ...
   Integer partsSz = (Integer) parts.size();
      ...
      MessageInterface::ShowMessage(wxT("In MarkPoint::InterpretAction, parts = \n"));
      for (Integer jj = 0; jj < partsSz; jj++)
         MessageInterface::ShowMessage(wxT("   %s\n"), parts.at(jj).c_str());
   #endif
   if (partsSz < 1) // 'MarkPoint' already found
      throw CommandException(wxT("Missing field in MarkPoint command"));
   for (Integer ii = 0; ii < partsSz; ii++)
      plotNameList.push_back(parts.at(ii));
   
   #ifdef DEBUG_MarkPoint
      MessageInterface::ShowMessage(wxT("Plots to be MarkPointed:\n"));
