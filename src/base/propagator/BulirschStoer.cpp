//$Id: BulirschStoer.cpp 9513 2011-04-30 21:23:06Z djcinsb $
//------------------------------------------------------------------------------
//                                 BulirschStoer
//------------------------------------------------------------------------------
 ...
 */
//------------------------------------------------------------------------------
BulirschStoer::BulirschStoer(const wxString &nomme) :
   Integrator                      (wxT("BulirschStoer"), nomme),
   depth                           (8),
 ...
 * the array wxT("d") in Numerical Recipes is not sized correctly.  It is set to be
...
bool BulirschStoer::SetMaximumDepth(Integer d)
{
   ...
   // Determine wxT("optimal") row for convergence
 ...
 * acting at intermediate points of the step (the so called wxT("substeps")).  The 
 * first and last steps are not generated from a midpoint formulation; hence 
 * this is a wxT("modified midpoint") extrapolation, implemented as described below.
 * 
       ...
       if (GmatMathUtil::Abs(stepSize) == minimumStep)
       {
          if (stopIfAccuracyViolated)
          {
             throw PropagatorException(
                   wxT("BulirschStoer: Accuracy settings will be violated with current step size values.\n"));
          }
          else
          {
 ...
 */
//------------------------------------------------------------------------------
wxString BulirschStoer::GetParameterText(const Integer id) const
{
 ...
 */
//------------------------------------------------------------------------------
Integer BulirschStoer::GetParameterID(const wxString &str) const
{
 ...
 */
//------------------------------------------------------------------------------
wxString BulirschStoer::GetParameterTypeString(const Integer id) const
{
   if (id >= MINIMUM_REDUCTION && id < BulirschStoerParamCount)
...
//------------------------------------------------------------------------------
Real BulirschStoer::GetRealParameter(const wxString &label) const
{
 ...
 */
//------------------------------------------------------------------------------
Real BulirschStoer::SetRealParameter(const wxString &label, const Real value)
{
