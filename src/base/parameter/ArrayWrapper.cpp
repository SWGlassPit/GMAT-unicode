 ...
 *
 * @param <aw>  ArrayWrapper instance to copy to create wxT("this") 
 * instance.
 */
//---------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
/**
 * @return ArrayWrapper value converted to wxString.
 *
 * @exception <GmatBaseException> thrown if this method is called.
 */
//------------------------------------------------------------------------------
wxString ArrayWrapper::ToString()
{
...
const StringArray& ArrayWrapper::GetRefObjectNames()
{
   ...
   refObjectNames.push_back(arrayName);
      ...
      MessageInterface::ShowMessage(wxT("ArrayWrapper:: Returning ref object names:\n"));
 ...
 */
//------------------------------------------------------------------------------
GmatBase* ArrayWrapper::GetRefObject(const wxString &name)
{
   return array;
}
...
bool ArrayWrapper::SetRefObject(GmatBase *obj)
{
   ...
   MessageInterface::ShowMessage
      (wxT("ArrayWrapper::SetRefObject() obj=<%p>, type='%s', name='%s', ")
   ...
   if ( (obj->IsOfType(wxT("Array"))) && (obj->GetName() == arrayName) )
   {
      array = (Array*) obj;
      #ifdef DEBUG_ARRAY_WRAPPER
         MessageInterface::ShowMessage(wxT("ArrayWrapper:: Setting array object %s\n"),
            arrayName.c_str());
      #endif
      isOk = true;
   }
   ...
   #ifdef DEBUG_ARRAY_WRAPPER
      MessageInterface::ShowMessage(
         wxT("ArrayWrapper:: Returning true from SetRefObject\n"));
   #endif
 ...
 */
//---------------------------------------------------------------------------
bool ArrayWrapper::RenameObject(const wxString &oldName, 
...
Real ArrayWrapper::EvaluateReal() const
{
   if (array->GetRowCount() == 1 && array->GetColCount() == 1)
      return array->GetRealParameter(wxT("SingleValue"), 0, 0);
   else
      throw GmatBaseException(
         wxT("ArrayWrapper::EvaluateReal() method not valid for wrapper of Array type.\n"));
}
...
{
   throw GmatBaseException(
      wxT("SetReal() method not valid for wrapper of Array type.\n"));
}
...
const Rmatrix& ArrayWrapper::EvaluateArray() const
{
   #ifdef DEBUG_ARRAY_WRAPPER
      MessageInterface::ShowMessage(
         wxT("ArrayWrapper::EvaluateArray called on array %s\n"), 
         arrayName.c_str());
   #endif
   if (array == NULL)
      throw ParameterException(
      wxT("Cannot return value of Array - object pointer is NULL\n"));
...
bool ArrayWrapper::SetArray(const Rmatrix &toValue)
{
   #ifdef DEBUG_ARRAY_WRAPPER
      MessageInterface::ShowMessage(
         wxT("ArrayWrapper::SetArray called on array %s\n"), 
         arrayName.c_str());
   #endif
   if (array == NULL)
      throw ParameterException(
      wxT("Cannot set value of Array - object pointer is NULL\n"));
   try
   {
   ...
   {
      wxString errmsg = wxT("Error setting Array value : ") + be.GetFullMessage();
      errmsg += wxT("\n");
...
void ArrayWrapper::SetupWrapper()
{
   #ifdef DEBUG_ARRAY_WRAPPER
   MessageInterface::ShowMessage
      (wxT("ArrayWrapper::SetupWrapper() description='%s'\n"), description.c_str());
   #endif
